...
# Data for Scree Plot
components <- 1:10
variance_explained <- c(26.77   , 47.72 ,   64.74 ,   72.07 ,   76.98   , 84.16,    87.60,    93.57    ,97.98 ,   100.00)
...
variance_explained <- c(28.49 ,   49.23   , 60.81   , 71.59  ,  77.85   , 83.18  ,  91.29  ,  94.19 ,   98.48 ,   100.00)
...
variance_explained <- c(30.08  ,  42.52   , 54.86  ,  71.28  ,  79.66 ,   85.15  ,  92.07   , 95.08  ,  98.22 ,   100.00)
...
pcr_pred <- predict(pcr_model, test1, ncomp =10)
y_test <- test1[, 1]
...
pcr_pred <- predict(pcr_model, test2, ncomp = 10)
y_test <- test2[, 1]
pcr_pred_numeric <- unlist(pcr_pred)
y_test_numeric <- unlist(y_test)
sqrt(mean((as.numeric(pcr_pred_numeric) - as.numeric(y_test_numeric))^2))
...
pcr_pred <- predict(pcr_model, test3, ncomp =7)
y_test <- test3[, 1]
pcr_pred_numeric <- unlist(pcr_pred)
#There is no negative lum
pcr_pred_numeric[pcr_pred_numeric < 0] <- 0
y_test_numeric <- unlist(y_test)
sqrt(mean((as.numeric(pcr_pred_numeric) - as.numeric(y_test_numeric))^2))
...
# Replace the last num_predictions values in temp_pred with your PCR predictions
combined_data3[(nrow(combined_data3) - num_predictions + 1):nrow(combined_data3), "lum_pred"] <- pcr_pred_numeric
...
pcr_pred <- predict(pcr_model, test4, ncomp = 9)
y_test <- test4[, 1]
pcr_pred_numeric <- unlist(pcr_pred)
y_test_numeric <- unlist(y_test)
sqrt(mean((as.numeric(pcr_pred_numeric) - as.numeric(y_test_numeric))^2))
...
pls_pred <- predict(pls_model, test1, ncomp =8)
y_test <- test1[, 1]
...
pls_pred <- predict(pls_model, test2, ncomp = 8)
y_test <- test2[, 1]
pls_pred_numeric <- unlist(pls_pred)
y_test_numeric <- unlist(y_test)
sqrt(mean((as.numeric(pls_pred_numeric) - as.numeric(y_test_numeric))^2))
...
pls_pred_numeric <- unlist(pls_pred)
y_test_numeric <- unlist(y_test)
pls_pred_numeric[pls_pred_numeric < 0] <- 0
sqrt(mean((as.numeric(pls_pred_numeric) - as.numeric(y_test_numeric))^2))

combined_data3$lum_pred2 <- NA
combined_data3[(nrow(combined_data3) - num_predictions + 1):nrow(combined_data3), "lum_pred2"] <- pls_pred_numeric
library(ggplot2)
...
pls_pred <- predict(pls_model, test4, ncomp = 8)
y_test <- test4[, 1]
pls_pred_numeric <- unlist(pls_pred)
y_test_numeric <- unlist(y_test)
sqrt(mean((as.numeric(pls_pred_numeric) - as.numeric(y_test_numeric))^2))
